from binning import normal_round_to_int, normal_round, bin_to_precision, bin_magnitudes

import pytest
    
    
@pytest.mark.parametrize("x, rounded_value", [(0.235, 0), (-0.235, 0), (-0.5, -1), (4.499, 4), (4.5, 5), (5.5, 6), (6.5, 7)])
def test_normal_round_to_int(x: float, rounded_value: int):
    y = normal_round_to_int(x)
    assert y == rounded_value


@pytest.mark.parametrize("x, n, rounded_value", [(0.235, 2, 0.24), (-0.235, 2, -0.24), (4.499, 2, 4.5), (4.5, 0, 5)])
def test_normal_round(x: float, n: int, rounded_value: float):
    y = normal_round(x, n)
    assert y == rounded_value


@pytest.mark.parametrize("x, delta_m, rounded_value", [(0.235, 0.1, 0.2), (-0.235, 0.2, -0.2), (4.499, 0.01, 4.5), (4.5, 0.2, 4.6)])
