...
where $b(m)$ is the b-value estimate of all magnitudes above $m$, $\sigma_b$ is its uncertainty, $\Delta m$ is the bin size of the discretized magnitudes and $K$ is the number of $m_c$ bins that are considered for stability. $L=K\cdot \Delta m$ is the length of the magnitude range considered for stability. Note that in SeismoStats, the input required for b-value estimation is $L$, not $K$.
